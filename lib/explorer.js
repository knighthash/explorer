...
module.exports = {
  ...
  get_hive_difficulty: function(cb) {
    var uri = base_url + 'gethivedifficulty';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  getbeecost: function(cb) {
    var uri = base_url + 'getbeecost';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
       ...
       } else if (vout[i].scriptPubKey.type == 'nonstandard' && vout[i].value != 0) {
            // checks for nonstandard bee creation
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: "Bee Creation", amount: amount_sat});
              loop.next();
            });
          } else {
